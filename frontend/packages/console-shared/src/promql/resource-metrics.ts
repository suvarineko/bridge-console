...
const podMetricsQueries = {
  ...
  [ResourceUtilizationQuery.CPU]: _.template("sum(node_namespace_pod_container:container_cpu_usage_seconds_total:sum_irate{pod='<%= name %>'}) BY (cluster,namespace,pod,prometheus)"),
...
const podControllerMetricsQueries = {
  ...
  [ResourceUtilizationQuery.FILESYSTEM]: _.template(
    "sum(container_fs_usage_bytes * on(pod) group_left(workload, workload_type) namespace_workload_pod:kube_pod_owner:relabel{workload='<%= name %>', workload_type='<%= type %>'}) by (pod)",
